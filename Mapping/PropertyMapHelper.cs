...
namespace PipeWeightTool.Mapping
{
    ...
    {
        public static void Map(Type type, DataRow row, PropertyInfo property, object entity)
        {
            List<string> columnsNames = AttributeHelper.GetDataNames(type, property.Name);
            ...
            foreach (var columnName in columnsNames)
            {
                if (!String.IsNullOrEmpty(columnName) && row.Table.Columns.Contains(columnName))
                {
                    ...
                    {
                        ParsePrimitive(property, entity, row[columnName]);
                        break;
                    }
                }
            }
